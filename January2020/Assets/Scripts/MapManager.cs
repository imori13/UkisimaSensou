        ...
        for (int i = 0; i < MapNode.Count; i++)
        {
            for (int j = 0; j < MapNode.Count; j++)
            {
                ...
                foreach (var node in MapNode)
                {
                    ...
                    float radius = 2.75f;
                    bool flag = MyMath.IsLineIntersectedCircle(road.PosS, road.PosE, MyMath.ConversionVector2(node.transform.position), radius);
                    if (!flag) continue;
    ...
    void SetPlayerNode()
    {
        ...
        for (int i = 0; i < PlayerCount; i++)
        {
            while (true)
            {
                ...
                void Hoge(ref List<Node> list, Node node, int num)
                {
                    foreach (var connect in node.ConnectNode)
                    {
                        ...
                        num++;
                        if (num >= 2) { continue; }
                        Hoge(ref list, connect, num);
                    }
                }
                bool flag = false;
                PlayerBaseNode.ToList().ForEach(b => { if (openList.Contains(b)) { flag = true; } });
                if (flag) continue;
